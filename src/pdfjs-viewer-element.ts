...
export class PdfjsViewerElement extends HTMLElement {
  ...
  private onIframeReady = debounce(
    async (callback: () => void) => {
...
${nameddest ? "&nameddest=" + nameddest : ""}`;
  ...
  private getCssThemeOption() {
    ...
    return Object.keys(ViewerCssTheme).includes(attrValue) ? ViewerCssTheme[attrValue] : ViewerCssTheme[DEFAULTS.viewerCssTheme];
  }
  ...
  public initialize = (): Promise<PdfjsViewerElementIframeWindow["PDFViewerApplication"]> =>
